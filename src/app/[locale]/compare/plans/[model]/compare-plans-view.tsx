...
export default function ComparePlansView({ locale, data }: ComparePlansViewProps) {
  ...
  // than an effect: collapsed groups render no plan markup at all, so expanding
  // after mount would mean the server-rendered HTML — the only thing a crawler
  // reads — contained zero plans.
  const [expandedProviders, setExpandedProviders] = useState<Set<string>>(
    () => new Set(
      [...(data.officialPlans || []), ...(data.thirdPartyPlans || [])]
        .map((p: any) => p.channel?.slug || p.channel?.id)
  ...
  const planGroups = React.useMemo(() => {
    ...
    allPlans.forEach((plan: any) => {
      // Must be deterministic: this now renders on the server too, so a random
      // key would differ between the server HTML and the client render and
      // break hydration. getPlanComparison always sets channel.slug (falling
      // back to 'unknown'), so the first branch effectively always wins.
      const providerId = plan.channel?.slug || plan.channel?.id || "unknown";
  ...
  // The newest `last_verified` across the plans on this page, as a plain
  // YYYY-MM-DD string. This line used to render `new Date().toLocaleDateString()`,
  // which claimed the prices were verified today no matter how stale they were —
  // and now that this component also renders on the server, a locale/timezone
  // dependent date would differ between the server HTML and the client and break
  // hydration. Omitted entirely when no plan carries a timestamp.
  const lastVerified = React.useMemo(() => {
    const stamps = [...(data.officialPlans || []), ...(data.thirdPartyPlans || [])]
      .map((p: any) => p.lastVerified)
      .filter(Boolean)
      .map((d: string | Date) => new Date(d))
      .filter((d: Date) => !Number.isNaN(d.getTime()));
    if (stamps.length === 0) return null;
    const newest = stamps.reduce((max, d) => (d > max ? d : max));
      ...
      <main className="container mx-auto px-4 py-8">
                              ...
                              <CardContent className="flex-1 flex flex-col">
                                ...
                                <div className="space-y-1 mb-2">
                                  ...
                                  <div className="flex items-center justify-between text-xs">
                                    ...
                                    <span className="font-medium">{model?.contextWindow ? `${(model.contextWindow / 1000).toFixed(0)}K` : "-"}</span>
                                  </div>
                                  <div className="flex items-center justify-between text-xs">
        ...
        <div className="bg-zinc-100 dark:bg-zinc-900 rounded-lg p-6">
          ...
          <ul className="space-y-2 text-sm text-zinc-600 dark:text-zinc-400">
            {lastVerified && (
              <li>
                • {locale === "zh" ? "价格最后核实：" : "Prices last verified: "}
                {lastVerified}
              </li>
            )}
            <li>• Exchange rate: 1 USD = 6.90 CNY (fixed reference rate)</li>
