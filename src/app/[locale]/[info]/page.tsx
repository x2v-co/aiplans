...
const content: Record<Locale, Record<InfoSlug, PageContent>> = {
  en: {
    ...
    methodology: {
      ...
      sections: [
        { heading: 'Pricing data sources', paragraphs: ['API token prices and subscription plan details come primarily from each provider’s own public pricing pages, API documentation, or provider-supplied machine-readable pricing endpoints. Cloud platforms (Azure OpenAI, AWS Bedrock, Vertex AI), aggregators (OpenRouter, SiliconFlow, Together AI and similar), and resellers such as XycAi are tracked as distinct channels and never presented as the model producer. When a channel exposes multiple public prices for the same model, such as XiuRouter service groups, XycAi account tiers, or OpenRouter upstream-provider endpoints, the comparison table uses a documented headline price and keeps the other variants attached to that channel.'], links: [{ label: 'Browse all tracked channels', url: '/en/api-pricing' }, { label: 'Browse subscription plans', url: '/en/plans' }] },
  ...
  zh: {
    ...
    methodology: {
      ...
      sections: [
        { heading: '价格数据来源', paragraphs: ['API token 价格和订阅套餐信息主要来自各厂商公开发布的定价页、API 文档或厂商提供的机器可读定价端点。云平台（Azure OpenAI、AWS Bedrock、Vertex AI）、聚合平台（OpenRouter、硅基流动、Together AI 等）和 XycAi 等转售渠道作为独立渠道记录，不会被描述为模型原厂。若同一渠道对同一模型公开多组价格，例如 XiuRouter 服务组、XycAi 账户档位或 OpenRouter 上游 provider endpoint，我们会使用有规则说明的 headline 价格进入主比较表，并把其他报价变体挂在该渠道下。'], links: [{ label: '查看全部渠道价格', url: '/zh/api-pricing' }, { label: '查看订阅套餐', url: '/zh/plans' }] },
