...
async function updateArenaScores() {
  ...
  for (const model of arenaData.top30) {
      ...
      if (foundModel) {
        ...
        if (existing) {
          ...
          if (model.score > existing.elo) {
            ...
            console.log(`   ✅ Updated ${foundModel.name} with higher ELO: ${model.score} (was ${existing.elo})`);
          }
        } else {
          updates.set(foundModel.id, { elo: model.score, source: [model.name] });
    ...
    if (!found) {
      console.log(`   ⚠️  Model not found for: ${model.name}`);
