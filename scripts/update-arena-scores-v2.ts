  ...
  for (const category of arenaData.categories) {
    ...
    for (const model of category.models) {
      ...
      for (const slug of possibleSlugs) {
        // Try to find the product by slug (exact match first)
        ...
        if (!error && products && products.length > 0) {
          ...
          if (existing) {
            ...
            if (model.arenaElo > existing.elo) {
              ...
              console.log(`   ✅ Updated ${product.name} with higher ELO: ${model.arenaElo} (was ${existing.elo})`);
            }
          } else {
            updates.set(product.id, { elo: model.arenaElo, source: [model.modelName] });
      ...
      if (!found) {
        console.log(`   ⚠️  Product not found for: ${model.rawModelName}`);
